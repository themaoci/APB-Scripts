   ...
   While $EXIT
	  ...
	  If $Switch_G == 1 Then ; 12 = ALT
		 If _IsPressed("68", $hDLL) Then
			...
			$switched_color = 0
		 EndIf
		 ; search for pixel on the left side
		 $FoundRed_0  = PixelSearch ($CenterX - $ScanX, $CenterY - 1, $CenterX, $CenterY + 1, 0xFF0000, 50)
		 If IsArray($FoundRed_0) Then
			; make sure same pixel on right side is in almost same color
			$rightSideCheck = $CenterX + ($CenterX - $FoundRed_0[0])
			; 0x3232FF
			$shade = PixelGetColor($rightSideCheck, $FoundRed_0[1])
			If ($shade > 0x0000CD) and ($shade < 0x3232FF) Then
			   Send('{h down}')
			   Sleep(2)
			   Send('{h up}')
			   Sleep(13)
